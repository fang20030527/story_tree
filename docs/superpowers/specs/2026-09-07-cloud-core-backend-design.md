...
1. 在 iOS 或 Android 上跑通“录入词义 → 生成文章 → 阅读／翻译 → 测验 → 进度更新”；Expo Web 只作编译与非鉴权界面预览，因为本轮长期安装凭据必须使用原生 SecureStore。
...
- `answer_kind`: `option` 或 `dont_know`
- `selected_option_id`，选择“我不知道”时为空
- `is_correct`
- `was_assisted`
- `elapsed_ms`
- `idempotency_key`
...
- 只有客户端实际展示翻译或提示时调用
- 服务端校验目标或段落确实属于该练习
- `word_hint` 在同一事务中记录辅助并返回该目标的中文义项；义项不会随未作答题目预先下发
...
- 必须带 `Idempotency-Key`
- 请求：`questionId`、`elapsedMs`，以及普通选项的 `selectedOptionId`；选择“我不知道”时显式提交 `answerKind: dont_know`
- 首次提交后返回正确性、正确选项、中文解释、英文释义和各干扰项说明
- “我不知道”保存为首次错误答案，不伪造一个已存在的错误选项
...
3. 要求模型生成 700–1,000 个英文单词的雅思路径文章，并返回标题、段落、每个目标的实际表面词形、题目、四个选项与解释。
...
用户点按高亮目标词时，客户端调用 `word_hint` 辅助接口；服务端先记录辅助证据，再返回并展示中文义项。
...
- Worker 会主动领取超过最终截止时间的任务，幂等地标记资源失败并释放额度；进程在两步之间崩溃时可在租约过期后安全重试。
