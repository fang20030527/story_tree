...
async function persistPreview(
  ...
  const row = await env.DB.prepare(`
    UPDATE article_imports SET status = 'preview_ready', preview_title = ?,
      preview_text = ?, preview_media_json = ?, word_count = ?, content_hash = ?, similarity_fingerprint = ?,
    ...
    RETURNING id
  `).bind(preview.title, preview.text, JSON.stringify(preview.media ?? []), preview.wordCount, preview.contentHash,
