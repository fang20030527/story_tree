...
async function findOwnedImport(
  ...
  try {
    row = await env.DB.prepare(
      'SELECT id, status, preview_text, preview_media_json FROM article_imports WHERE id = ? AND user_id = ? LIMIT 1',
...
async function editPreview(
  ...
  const media = current.preview_text === normalized.text
    ? parseStoredMedia(current.preview_media_json)
    : [];
  ...
  await commitMutation(env, {
    ...
    updateSql: `
      ...
      SET preview_title = ?, preview_text = ?, preview_media_json = ?, word_count = ?, content_hash = ?,
    ...
    `,
    updateValues: [
      normalized.title, normalized.text, JSON.stringify(media), normalized.wordCount,
