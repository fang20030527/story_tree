...
async function commitConfirmation(
  ...
  statements.push(env.DB.prepare(`
    ...
    WHERE id = ? AND user_id = ? AND status = 'preview_ready'
      AND preview_title = ? AND preview_text = ? AND word_count = ?
  ...
  `).bind(
    ...
    input.current.preview_text, input.current.word_count,
  ...
  if (input.resolution.createArticle) {
    statements.push(env.DB.prepare(`
      INSERT INTO imported_articles
        (id, user_id, source_kind, source_url, title, word_count,
    ...
    `).bind(
      ...
      input.content.title, input.content.wordCount, input.content.contentHash,
...
async function confirmImport(
  ...
  if (existing) {
    ...
    await commitConfirmation(env, {
      userId, importId, key, requestHash, current, content, resolution,
    });
  }
